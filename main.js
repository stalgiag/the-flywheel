...
document.addEventListener("DOMContentLoaded", function () {
    if (mobileAndTabletcheck()) {
        const block = document.querySelector('#mobile-site-block');
...
function flashingLive() {
    liveLogos = document.querySelectorAll(".live-logo");
    setInterval(() => {
        for (live of liveLogos) {
            live.style.visibility = live.style.visibility === 'hidden' ? 'visible' : 'hidden';
        }
    }, 500);
}